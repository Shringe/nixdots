      ...
      settings =
        ...
        with config.nixosModules.server.services;
        {
          ...
          endpoints = flatten [
            ...
            (mkMonitor "Traccar" traccar)
            (mkMonitor "Wallos" docker.wallos)
            (mkMonitor "Paperless" paperless)
            (mkMonitor "Collabora" collabora)
            (mkMonitor "SearXNG" searxng)
            (mkMonitor "Home Assistant" homeAssistant)
            (mkMonitor "Homepage" homepage)
          ];
        };
    };
  };
